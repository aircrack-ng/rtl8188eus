...
#define PPPOE_TAGS(ph)      ((unsigned char *)((ph) + 1))
#define PPPOE_TAG_DATA(tag) ((unsigned char *)((tag) + 1))
#else
#define PPPOE_TAGS(ph)      ((unsigned char *)(ph)->tag)
#define PPPOE_TAG_DATA(tag) ((unsigned char *)(tag)->tag_data)
#endif
#endif
...
static __inline__ unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	start_ptr = cur_ptr = PPPOE_TAGS(ph);
...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove((PPPOE_TAGS(ph) + data_len), PPPOE_TAGS(ph), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
#if (defined __GNUC__) && (__GNUC__ > 10)
    #pragma GCC diagnostic ignored "-Wstringop-overread"
#endif
	memcpy(PPPOE_TAGS(ph), tag, data_len);
#if (defined __GNUC__) && (__GNUC__ > 10)
